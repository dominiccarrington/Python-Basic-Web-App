...
class Router(object):
    ...
    def get_path_from_name(self, name, **args):
        if name not in self.named_routes.keys():
            raise ValueError("Piss off mate... " + name + " does not exist!")

        s = self.named_routes[name]
        for key, value in args.items():
            s = s.replace("{" + str(key) + "}", str(value))

        checkPtn = re.compile(r"{[^/]*}")
        if checkPtn.search(s):
            logger.error("Not all options filled for " + name)
            s = None
